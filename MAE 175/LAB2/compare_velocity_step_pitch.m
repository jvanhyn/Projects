...
% due to a step on pitch motor
%
% Written by R.A. de Callafon, Dept. of MAE, UCSD (2015-2023)
% Report errors in this software to <callafon@ucsd.edu>

% read pitch angle data file due to step on pitch motor
data_file = 'Pitch_Angle_Pitch_Motor_Step1.txt';
...
% read yaw angle data file due to step on pitch motor
data_file = 'Yaw_Angle_Pitch_Motor_Step1.txt';
disp(['Reading ' data_file '...'])
f=fopen(data_file,'r');
tline = fgetl(f);
k=1;
...
% simulate step response of velocity due to step on pitch motor
t_sim = [0:1/200:4-1/200];
N_sim = length(t_sim);
start_of_step = 60; % in number of samples of t_sim
pitch_motor = [zeros(start_of_step,1);ones(N_sim-start_of_step,1)];
yaw_motor = [zeros(N_sim,1)];
...
set(l,'linewidth',2);
title('pitch angular velocity due to step on pitch motor')
...
set(l,'linewidth',2);
title('yaw angular velocity due to step on pitch motor')
